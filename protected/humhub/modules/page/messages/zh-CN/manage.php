<?php
/**
 * Message translations.
 *
 * This file is automatically generated by 'yii message/extract' command.
 * It contains the localizable messages extracted from source code.
 * You may modify this file by translating the extracted messages.
 *
 * Each array element represents the translation (value) of a message (key).
 * If the value is empty, the message is considered as not translated.
 * Messages that no longer need translation will have their translations
 * enclosed between a pair of '@@' marks.
 *
 * Message string can be used with plural forms format. Check i18n section
 * of the guide for details.
 *
 * NOTE: this file must be saved in UTF-8 encoding.
 */
return [
    '<strong>Create</strong> new page' => '<strong>创建</strong> 新空间',
    '<strong>Manage</strong> members' => '<strong>管理</strong> 成员',
    '<strong>Members</strong>' => '<strong>成员</strong>',
    '<strong>Security</strong> settings' => '<strong>安全</strong> 设置',
    '<strong>Page</strong> Modules' => '<strong>空间</strong> 模块',
    '<strong>Page</strong> settings' => '<strong>空间</strong> 设置',
    'Actions' => '动作',
    'Activated' => '启用',
    'Add <strong>Modules</strong>' => '添加 <strong>模块</strong>',
    'Advanced access settings' => '访问权限设置',
    'Archive' => '存档',
    'Are you sure, that you want to delete this page? All published content will be removed!' => '你确定，你要删除这个空间？所有发布的内容将被删除！',
    'Are you sure? *ALL* module data for this page will be deleted!' => '你确定？这个模块的所有的数据将会被删除！',
    'As owner of this page you can transfer this role to another administrator in page.' => '作为这个空间的所有者，你可以把这个角色转移到另一个空间管理员。',
    'Cancel Membership' => '取消成员',
    'Choose if new content should be public or private by default' => '选择新的内容默认应该是公开或是私有',
    'Choose the kind of membership you want to provide for this workspace.' => '选择你想为这个空间的哪种会员。',
    'Choose the security level for this workspace to define the visibleness.' => '选择这个空间的安全级别来定义可见性。',
    'Configure' => '配置',
    'Currently there are no modules available for this page!' => '该空间当前没有生效模块',
    'Delete' => '删除',
    'Disable' => '失效',
    'Don\'t receive notifications for new content' => '不接收新内容通知',
    'Enable' => '生效',
    'Enhance this page with modules.' => '用模块来增强这个空间。',
    'Hide posts on dashboard' => '在控制面板隐藏文章',
    'Members' => '成员',
    'Modules' => '模块',
    'Next' => '下一步',
    'Owner' => '拥有者',
    'Pending Approvals' => '待处理审核',
    'Pending Invites' => '待处理邀请',
    'Permissions are assigned to different user-roles. To edit a permission, select the user-role you want to edit and change the drop-down value of the given permission.' => '权限分配给不同的用户角色。要编辑权限，选择要编辑的用户角色，并更改给定权限的下拉值。',
    'Receive Notifications for new content' => '接收新内容通知',
    'Role' => '角色',
    'Security' => '安全',
    'Send & decline' => '发送并拒绝',
    'Show all' => '展示所有',
    'Show posts on dashboard' => '在控制面板展示文章',
    'Page name' => '空间名字',
    'Page owner' => '空间拥有者',
    'Stream (Default)' => '流 (默认)',
    'The url contains illegal characters!' => 'url包含非法字符！',
    'This option will hide new content from this page at your dashboard' => '此选项将在控制面板上隐藏这个空间的新内容',
    'This option will show new content from this page at your dashboard' => '此选项将在控制面板上展示这个空间的新内容',
    'Transfer ownership' => '转让拥有者',
    'Unarchive' => '取消存档',
    'e.g. example for {baseUrl}/s/example' => '例如 转给example 用 {baseUrl}/s/example',
    'never' => '从不',
    'page description' => '空间描述',
    'the default start page of this page for members' => '这个空间成员默认的开始页',
    'the default start page of this page for visitors' => '这个空间游客默认的开始页',
    '-' => '',
    'Invited By' => '',
    'Please type the name of the page to proceed.' => '',
    'Remove from page' => '',
];
