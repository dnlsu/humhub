<?php
/**
 * Message translations.
 *
 * This file is automatically generated by 'yii message/extract' command.
 * It contains the localizable messages extracted from source code.
 * You may modify this file by translating the extracted messages.
 *
 * Each array element represents the translation (value) of a message (key).
 * If the value is empty, the message is considered as not translated.
 * Messages that no longer need translation will have their translations
 * enclosed between a pair of '@@' marks.
 *
 * Message string can be used with plural forms format. Check i18n section
 * of the guide for details.
 *
 * NOTE: this file must be saved in UTF-8 encoding.
 */
return [
    '<b>This page is still empty!</b>' => '<b>Tento prostor je stále prázdný!</b>',
    '<b>This page is still empty!</b><br>Start by posting something here...' => '<b>Zatím zde nejsou žádné příspěvky.</b><br>Napište první...',
    '<b>You are not member of this page and there is no public content, yet!</b>' => '<B> Nejste členem tohoto prostoru a zatím zde není žádný veřejný obsah! </ B>',
    '<strong>Confirm</strong> image deletion' => '<strong>Potvrzení </strong> smazání obrázku',
    '<strong>Invite</strong> members' => '<strong>Pozvat</strong> členy',
    '<strong>New</strong> member request' => '<strong>Nová</strong> žádost o členství',
    '<strong>Request</strong> page membership' => '<strong>Žádost</strong> o členství v prostoru',
    '<strong>Page</strong> followers' => '<strong>Sledující prostoru</strong>',
    '<strong>Page</strong> members' => '<strong>Členové</strong> prostoru',
    '<strong>Page</strong> menu' => '<strong>Menu</strong> prostoru',
    'Accept' => 'Přijmout',
    'Accept Invite' => 'Přijmout pozvánku',
    'Administrators' => 'Administrátoři',
    'Application message' => 'Zpráva aplikace',
    'Archived' => 'Archivováno',
    'As owner you cannot revoke your membership!' => 'Nemůžete si zrušit členství tomto prostoru, protože jste jeho vlastník!',
    'Become member' => 'Stát se členem',
    'Cancel' => 'Zrušit',
    'Cancel pending membership application' => 'Zrušit čekající žádost o členství',
    'Close' => 'Zavřít',
    'Color' => 'Barva',
    'Could not request membership!' => 'Není možné požádat o členství!',
    'Created At' => 'Vytvořeno',
    'Created By' => 'Vytvořil(a)',
    'Decline' => 'Nechci se zúčastnit',
    'Default' => 'Výchozí',
    'Default content visibility' => 'Výchozí viditelnost příspěvku',
    'Delete' => 'Smazat',
    'Description' => 'Popis',
    'Do you really want to delete your profile image?' => 'Opravdu chcete smazat svůj profilový obrázek?',
    'Do you really want to delete your title image?' => 'Opravdu chcete smazat svůj úvodní obrázek?',
    'Done' => 'Hotovo',
    'Email addresses' => 'E-mailové adresy',
    'Everyone can enter' => 'Každý se může přidat',
    'Follow' => 'Sledovat',
    'Followers' => 'sleduje mě',
    'Homepage' => 'Domovská stránka',
    'Homepage (Guests)' => 'Domovská stránka(Host)',
    'Invite' => 'Pozvat',
    'Invite and request' => 'Pozváním nebo požádáním',
    'Invite by email' => 'Pozvat e-mailem',
    'Invites' => 'Pozvánky',
    'Join Policy' => 'Jak se lze stát členem?',
    'Last Visit' => 'Poslední návštěva ',
    'Login' => 'Přihlásit se',
    'Members' => 'Členové',
    'Moderators' => 'Moderátoři',
    'My Page List' => 'Můj seznam prostorů',
    'My page summary' => 'Můj souhrn z prostorů',
    'Name' => 'Jméno skupiny',
    'New user by e-mail (comma separated)' => 'Pozvání uživatele pomocí e-mailu (adresy oddělte čárkou)',
    'New user?' => 'Nový uživatel?',
    'No pages found.' => 'Žádné prostory nebyly nalezeny.',
    'Only by invite' => 'Jen pozváním',
    'Originator User ID' => 'ID uživatele ',
    'Owner' => 'Vlastník',
    'Pick users' => 'Vybrat uživatele',
    'Please shortly introduce yourself, to become a approved member of this workspace.' => 'Stručně se prosím představte, aby bylo možné schválit vaše členství v tomto prostoru.',
    'Please shortly introduce yourself, to become an approved member of this page.' => 'Stručně se prosím představte, aby bylo možné schválit vaše členství v tomto prostoru.',
    'Posts' => 'Příspěvky',
    'Private' => 'Soukromé',
    'Private (Invisible)' => 'Neveřejné (skryté)',
    'Public' => 'Veřejné',
    'Public (Members & Guests)' => 'Veřejné (pro členy a hosty)',
    'Public (Members only)' => 'Veřejné (pouze pro členy)',
    'Public (Registered users only)' => 'Veřejné (pouze registrovaní uživatelé)',
    'Request Message' => 'Žádost o zprávu',
    'Request membership' => 'Požádat o členství',
    'Request workspace membership' => 'Žádost o členství v prostoru',
    'Send' => 'Odeslat',
    'Settings' => 'Nastavení',
    'Show all' => 'Zobrazit vše',
    'Sorry, you are not allowed to leave this page!' => 'Omlouváme se, nemáte dovoleno opustit tento prostor!',
    'Page' => 'Prostor',
    'Page directory' => 'Adresář prostoru',
    'Page is invisible!' => 'Tento prostor je skrytý!',
    'Pages' => 'Prostory',
    'Status' => 'Stav',
    'Stream' => 'Příspěvky',
    'Tags' => 'Štítky',
    'There is no pending invite!' => 'Momentálně zde není žádná pozvánka čekající na přijetí!',
    'This action is only available for workspace members!' => 'Tato akce je možná pouze pro členy prostoru!',
    'This user is already a member of this page.' => 'Tento uživatel je již členem tohoto prostoru.',
    'This user is not a member of this page.' => 'Tento uživatel není členem tohoto prostoru.',
    'To invite users to this page, please type their names below to find and pick them.' => 'Pokud chcete pozvat do tohoto prostoru další uživatele, začněte psát jejich jména a poté je vyberte ze seznamu.',
    'Unfollow' => 'Přestat sledovat',
    'Updated At' => 'Aktualizováno',
    'Updated By' => 'Aktualizoval/a',
    'Updated by' => 'Aktualizoval(a)',
    'User \'{username}\' is already a member of this page!' => 'Uživatele \'{username}\' je již členem tohoto prostoru!',
    'User \'{username}\' is already an applicant of this page!' => 'Uživatel \'{username}\' je již žadatelem tohoto prostoru!',
    'User not found!' => 'Uživatel nebyl nalezen!',
    'Users' => 'Uživatelé',
    'Visibility' => 'Viditelnost',
    'Visible for all (members and guests)' => 'Viditelné pro všechny',
    'You are not allowed to join this page!' => 'Není možné se připojit k tomuto prostoru!',
    'You can also invite external users, which are not registered now. Just add their e-mail addresses separated by comma.' => 'Můžete také pozvat externí uživatele, kteří zatím nejsou zaregistrovaní. Stačit napsat jejich e-mailové adresy (oddělené čárkou).',
    'You cannot create private visible pages!' => 'Nemáte oprávnění vytvářet neveřejné prostory!',
    'You cannot create public visible pages!' => 'Nemáte oprávnění vytvářet veřejné prostory!',
    'You need to login to view contents of this page!' => 'Musíte se přihlásit pro zobrazení tohoto prostoru!',
    'Your request was successfully submitted to the page administrators.' => 'Váše žádost byla uspěšně předána správci prostoru.',
    'Your request was successfully submitted to the workspace administrators.' => 'Váše žádost byla uspěšně předána správci prostoru.',
    '{email} is already registered!' => 'E-mailová adresa {email} již v databázi existuje!',
    '{email} is not valid!' => 'E-mailová adresa {email} není platná!',
    'Add users without invitation' => '',
    'Administrator:' => '',
    'Decline Invite' => '',
    'Moderator:' => '',
    'Owner:' => '',
    'Select all registered users' => '',
    'Settings could not be saved!' => '',
    'User invitations have been added to the queue' => '',
    'User memberships have been added to the queue' => '',
    'Users has been invited.' => '',
    '{count} members' => '',
];
