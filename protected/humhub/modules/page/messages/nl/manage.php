<?php
return array (
  '-' => '-',
  '<strong>Create</strong> new page' => '<strong>Maak</strong> nieuwe ruimte',
  '<strong>Manage</strong> members' => '<strong>Beheer</strong> leden',
  '<strong>Members</strong>' => '<strong>Leden</strong>',
  '<strong>Security</strong> settings' => '<strong>Veiligheids</strong> instellingen',
  '<strong>Page</strong> Modules' => '<strong>Ruimte</strong> Modules',
  '<strong>Page</strong> settings' => '<strong>Ruimte</strong> instellingen',
  'Actions' => 'Acties',
  'Activated' => 'Geactiveerd',
  'Add <strong>Modules</strong>' => '<strong>Modules</strong> toevoegen',
  'Advanced access settings' => 'Geavanceerde instellingen voor toegang',
  'Archive' => 'Archiveren',
  'Are you sure, that you want to delete this page? All published content will be removed!' => 'Bent u zeker dat u deze ruimte wilt verwijderen? Alle gepubliceerde berichten zullen verwijderd worden!',
  'Are you sure? *ALL* module data for this page will be deleted!' => 'Bent u zeker? *ALLE* module data voor deze ruimte zal verwijderd worden!',
  'As owner of this page you can transfer this role to another administrator in page.' => 'Als eigenaar van deze ruimte  kunt u deze rol overdragen naar een andere administrator in deze ruimte.',
  'Cancel Membership' => 'Lidmaatschap beëindigen',
  'Choose if new content should be public or private by default' => 'Kies of nieuwe content standaard openbaar of privé is',
  'Choose the kind of membership you want to provide for this workspace.' => 'Kies het soort lidmaatschap dat je wilt voorzien voor deze ruimte.',
  'Choose the security level for this workspace to define the visibleness.' => 'Kies het beveiligingsniveau voor deze ruimte om de zichtbaarheid te bepalen.',
  'Configure' => 'Configureren',
  'Currently there are no modules available for this page!' => 'Er zijn momenteel geen modules beschikbaar voor deze ruimte!',
  'Delete' => 'Verwijder',
  'Disable' => 'Deactiveren',
  'Don\'t receive notifications for new content' => 'Ontvang geen meldingen voor nieuwe inhoud.',
  'Enable' => 'Activeren',
  'Enhance this page with modules.' => 'Breid deze ruimte uit met modules.',
  'Hide posts on dashboard' => 'Verberg berichten op het dashboard',
  'Invited By' => 'Uitgenodigd door',
  'Members' => 'Leden',
  'Modules' => 'Modules',
  'Next' => 'Volgende',
  'Owner' => 'Eigenaar',
  'Pending Approvals' => 'Wachtende verzoeken',
  'Pending Invites' => 'Wachtende uitnodigingen',
  'Permissions are assigned to different user-roles. To edit a permission, select the user-role you want to edit and change the drop-down value of the given permission.' => 'Permissies zijn toegewezen aan verschillende groepen gebruikers. Om een permissie aan te passen, klik je op de groep die je wilt bewerken en selecteer je in het drop-down menu de gewenste permissie.',
  'Please type the name of the page to proceed.' => 'Typ de naam van de ruimte om door te gaan.',
  'Receive Notifications for new content' => 'Ontvang meldingen voor nieuwe inhoud.',
  'Remove from page' => 'Verwijderen uit de ruimte',
  'Role' => 'Rol',
  'Security' => 'Veiligheid',
  'Send & decline' => 'Verzenden &amp; afwijzen',
  'Show all' => 'Toon alles',
  'Show posts on dashboard' => 'Berichten weergeven op het dashboard',
  'Page name' => 'Ruimte naam',
  'Page owner' => 'Ruimte eigenaar',
  'Stream (Default)' => 'Stream (Standaard)',
  'The url contains illegal characters!' => 'De URL bevat niet toegestane tekens,',
  'This option will hide new content from this page at your dashboard' => 'Deze optie zal nieuwe inhoud van deze ruimte verbergen in uw dashboard',
  'This option will show new content from this page at your dashboard' => 'Deze optie zal nieuwe inhoud van deze ruimte weergeven in uw dashboard',
  'Transfer ownership' => 'Eigenaarschap overdragen',
  'Unarchive' => 'Dearchiveer',
  'e.g. example for {baseUrl}/s/example' => 'Dat is bijvoorbeeld voor {baseUrl}/s/voorbeeld',
  'never' => 'nooit',
  'page description' => 'Ruimte beschrijving',
  'the default start page of this page for members' => 'De standaard startpagina voor leden van deze ruimte.',
  'the default start page of this page for visitors' => 'De standaard startpagina voor bezoekers van deze ruimte.',
);
