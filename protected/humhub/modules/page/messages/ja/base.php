<?php
/**
 * Message translations.
 *
 * This file is automatically generated by 'yii message/extract' command.
 * It contains the localizable messages extracted from source code.
 * You may modify this file by translating the extracted messages.
 *
 * Each array element represents the translation (value) of a message (key).
 * If the value is empty, the message is considered as not translated.
 * Messages that no longer need translation will have their translations
 * enclosed between a pair of '@@' marks.
 *
 * Message string can be used with plural forms format. Check i18n section
 * of the guide for details.
 *
 * NOTE: this file must be saved in UTF-8 encoding.
 */
return [
    '<b>This page is still empty!</b>' => '<b>このスペースにはまだ何も書かれていません。</b>',
    '<b>This page is still empty!</b><br>Start by posting something here...' => '<b>このスペースにはまだ何も書かれていません</b><br />何か投稿してみてください…',
    '<b>You are not member of this page and there is no public content, yet!</b>' => '<b>あなたはこのスペースのメンバーではなく、閲覧できる公開投稿がまだありません。</b>',
    '<strong>Confirm</strong> image deletion' => '<strong>確認</strong> 画像の削除',
    '<strong>Invite</strong> members' => '<strong>メンバーを招待</strong>',
    '<strong>New</strong> member request' => '<strong>新規</strong> メンバーのリクエスト',
    '<strong>Request</strong> page membership' => '<strong>リクエスト</strong> スペースメンバー',
    '<strong>Page</strong> followers' => '<strong>スペース</strong> のフォロワー',
    '<strong>Page</strong> members' => '<strong>スペース</strong> メンバー',
    '<strong>Page</strong> menu' => '<strong>スペース</strong> メニュー',
    'Accept' => '同意',
    'Accept Invite' => '招待を受ける',
    'Add users without invitation' => '招待なしでメンバーを追加する',
    'Administrator:' => '管理者：',
    'Administrators' => '管理者',
    'Application message' => 'アプリケーションメッセージ',
    'Archived' => 'アーカイブ',
    'As owner you cannot revoke your membership!' => 'あなたは所有者として自身のメンバーシップを取り消すことはできません。',
    'Become member' => 'メンバーになる',
    'Cancel' => 'キャンセル',
    'Close' => '閉じる',
    'Color' => '色',
    'Could not request membership!' => 'メンバーシップを申し込むことができませんでした。',
    'Created At' => '作成',
    'Created By' => '作成者',
    'Decline' => '辞退',
    'Default' => 'デフォルト',
    'Default content visibility' => 'コンテンツの見え方',
    'Delete' => '削除',
    'Description' => '説明',
    'Do you really want to delete your profile image?' => '本当にあなたのプロフィール画像を削除しますか？',
    'Done' => '実行',
    'Email addresses' => 'Eメールアドレス',
    'Everyone can enter' => '誰でも参加できます',
    'Follow' => 'フォロー',
    'Followers' => 'フォロワー',
    'Homepage' => 'ホームページ',
    'Homepage (Guests)' => 'ホームページ（ゲスト）',
    'Invite' => '招待',
    'Invite and request' => '招待とリクエスト',
    'Invite by email' => 'Eメールアドレスで招待する',
    'Invites' => '招待',
    'Join Policy' => '参加方法',
    'Last Visit' => '最後の訪問',
    'Login' => 'ログイン',
    'Members' => 'メンバー',
    'Moderator:' => 'モデレーター：',
    'Moderators' => 'モデレーター',
    'My Page List' => 'スペースのリスト',
    'My page summary' => 'スペースのサマリー',
    'Name' => '名前',
    'New user by e-mail (comma separated)' => 'メールで新規ユーザー登録（カンマ区切り）',
    'No pages found.' => 'スペースが見つかりませんでした',
    'Only by invite' => '招待制',
    'Owner' => '所有者',
    'Owner:' => 'オーナー：',
    'Pick users' => 'ユーザー選択',
    'Posts' => '投稿',
    'Private' => 'プライベート',
    'Private (Invisible)' => 'プライベート（閲覧制限）',
    'Public' => '公開',
    'Public (Members & Guests)' => '公開（メンバーとゲスト）',
    'Public (Members only)' => '公開（メンバー限定）',
    'Public (Registered users only)' => '公開（登録ユーザー限定）',
    'Send' => '送信',
    'Settings' => '設定',
    'Show all' => 'すべて見る',
    'Page' => 'スペース',
    'Page directory' => 'スペースのディレクトリ',
    'Page is invisible!' => 'スペースは不可視です。',
    'Pages' => 'スペース',
    'Status' => 'ステータス',
    'Stream' => 'ストリーム',
    'Tags' => 'タグ',
    'There is no pending invite!' => '保留中の招待はありません。',
    'This action is only available for workspace members!' => 'このアクションは、ワークスペースメンバーにのみ使用可能です。',
    'To invite users to this page, please type their names below to find and pick them.' => 'このスペースにメンバーを招待します。テキストボックに招待したいメンバーの名前を入力してください。',
    'Unfollow' => 'アンフォロー',
    'Updated At' => '更新',
    'Updated By' => '更新者',
    'Updated by' => '更新されました。',
    'User \'{username}\' is already a member of this page!' => 'ユーザー \'{username}\' はすでにメンバーです。',
    'User not found!' => 'ユーザーが見つかりませんでした。',
    'Users' => 'ユーザー',
    'Visibility' => '可視性',
    'You are not allowed to join this page!' => 'あなたは、このスペースへの参加を許可されていません。',
    'You cannot create private visible pages!' => 'あなたはプライベートスペースを作成することはできません。',
    'You cannot create public visible pages!' => 'あなたは公開スペースを作成することはできません。',
    'You need to login to view contents of this page!' => 'このスペースを閲覧するにはログインが必要です',
    '{count} members' => '{count} メンバー',
    '{email} is already registered!' => '{email}はすでに登録されています。',
    '{email} is not valid!' => '{email}は有効ではありません。',
    'Cancel pending membership application' => '',
    'Decline Invite' => '',
    'Do you really want to delete your title image?' => '',
    'New user?' => '',
    'Originator User ID' => '',
    'Please shortly introduce yourself, to become a approved member of this workspace.' => '',
    'Please shortly introduce yourself, to become an approved member of this page.' => '',
    'Request Message' => '',
    'Request membership' => '',
    'Request workspace membership' => '',
    'Select all registered users' => '',
    'Settings could not be saved!' => '',
    'Sorry, you are not allowed to leave this page!' => '',
    'This user is already a member of this page.' => '',
    'This user is not a member of this page.' => '',
    'User \'{username}\' is already an applicant of this page!' => '',
    'User invitations have been added to the queue' => '',
    'User memberships have been added to the queue' => '',
    'Users has been invited.' => '',
    'Visible for all (members and guests)' => '',
    'You can also invite external users, which are not registered now. Just add their e-mail addresses separated by comma.' => '',
    'Your request was successfully submitted to the page administrators.' => '',
    'Your request was successfully submitted to the workspace administrators.' => '',
];
