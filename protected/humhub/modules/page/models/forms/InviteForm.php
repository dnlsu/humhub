...
namespace humhub\modules\page\models\forms;
...
class InviteForm extends Model
{
    ...
    /**
     * Indicate for add users to page without invite notification
     * @var bool
     */
    ...
    {
        return [
            'invite' => Yii::t('PageModule.base', 'Invites'),
            'inviteExternal' => Yii::t('PageModule.base', 'New user by e-mail (comma separated)'),
        ];
    }
    ...
    public function forceInvite() {
        Yii::$app->queue->push(new AddUsersToPageJob([
    ...
    public function inviteMembers() {
        foreach ($this->getInvites() as $user) {
            $this->page->inviteMember($user->id, Yii::$app->user->id);
        }
    }
    ...
    public function inviteExternal()
    {
        if ($this->canInviteExternal()) {
            foreach ($this->getInvitesExternal() as $email) {
                $this->page->inviteMemberByEMail($email, Yii::$app->user->id);
            }
        }
    }
    ...
    public function checkInvite($attribute, $params)
    {
        ...
        if ($this->$attribute != '') {
            ...
            foreach ($invites as $userGuid) {
                ...
                if ($user === null) {
                    $this->addError($attribute, Yii::t('PageModule.base', 'User not found!'));
                    continue;
                }

                $membership = Membership::findOne(['page_id' => $this->page->id, 'user_id' => $user->id]);
                ...
                if ($membership && $membership->status == Membership::STATUS_MEMBER) {
                    ...
                    $this->addError($attribute, Yii::t('PageModule.base',
    ...
    /**
     * Checks a comma separated list of e-mails which should invited to page.
            ...
            foreach ($emails as $email) {
                ...
                if (!$validator->validate($email)) {
                    $this->addError($attribute,
                        Yii::t('PageModule.base', "{email} is not valid!", ["{email}" => $email]));
                    continue;
                }
                ...
                if ($user != null) {
                    $this->addError($attribute,
                        Yii::t('PageModule.base', "{email} is already registered!",
                            ["{email}" => $email]));
                    continue;
                }
